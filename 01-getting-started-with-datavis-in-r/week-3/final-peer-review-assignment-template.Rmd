...
```{r,problem4}
boxplot(dat$faminc_new, main = "Bin He", xlab = "Family Income")
```
...
dat %>% 
  filter(gender == 1, region <= 2, marstat == 1, newsint == 1) %>% 
  str()
```
...
dat1 <- dat %>% 
  mutate(tax_scale = 4- CC18_325a - CC18_325d)
head(dat1$tax_scale, n = 20)
```
...
```{r problem10}
dat %>% 
  filter(investor == 2, faminc_new %in% 5:10) %>% 
  summarize(mean = mean(pew_religimp), median = median(pew_religimp), sd = sd(pew_religimp))
```
...
```{r problem11}
dat %>% summarise(mean = mean(faminc_new), median = median(faminc_new), sd = sd(faminc_new)) %>% knitr::kable()
```
...
qplot(pid7, data = dat, xlab = "Seven Point Party ID", ylab = "Count", geom = "histogram")
```