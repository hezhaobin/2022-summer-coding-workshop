...
cel %>% filter(congress == 115) %>% 
  ...
  ggplot(aes(x = dwnom1, y = all_pass)) + geom_point(aes(color = Gender)) + xlab("Ideology") + ylab("Bills Passed")
```
...
cel %>% filter(congress == 115) %>% 
  ...
  ggplot(aes(x = votepct, y = all_pass)) + geom_point(aes(color = Gender)) + 
  scale_color_manual(values = c("Female" = "orange", "Male" = "green")) +
  facet_wrap(~majority) +
  xlab("Vote Percentage") + ylab("Bills Passed")
```
...
cel %>% filter(congress == 115) %>% 
  ...
  xlab("Majority or Minority") + ylab("Legislative Effectiveness")
